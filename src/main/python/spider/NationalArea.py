...
# ---------------------------------------------
# @file NationalArea.py
# @description NationalArea
# @author WcJun
# @date 2021/01/08
# ---------------------------------------------
...
# 递归抓取下一页面
def spider_next(target_file, url, lev):
    if lev == 2:
        spider_class = "city"
    elif lev == 3:
        spider_class = "county"
    ...
    for item in get_html(url).select("tr." + spider_class + "tr"):
        ...
        print(content2)
        target_file.write(content2 + "\n")
        if item_href is not None:
            spider_next(target_file, get_prefix(url) + item_href, lev + 1)
...
def main():
    ...
    target_file = open("nation-area-2021.txt", "w", encoding="utf-8")
    try:
        for province in province_list:
            ...
            spider_next(target_file, get_prefix(province_url) + href, 2)
    finally:
        target_file.close()
